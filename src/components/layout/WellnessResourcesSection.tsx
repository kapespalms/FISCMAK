...
function ResourceRow({ resource }: { resource: (typeof PRIMARY_CRISIS_RESOURCES)[number] }) {
  return (
    <li className="border-b border-cx-forest-dark/10 py-3 last:border-0">
      <p className="text-sm font-medium text-cx-text">{resource.label}</p>
      <p className="mt-0.5 text-sm text-cx-text/75">{formatResourceContact(resource)}</p>
      {resource.url && (
        <a
          href={resource.url}
          target="_blank"
          rel="noopener noreferrer"
          className="mt-1 inline-block text-xs font-medium text-cx-text underline underline-offset-2 hover:text-cx-text/80"
        >
          Learn more
        </a>
      )}
