...
export function ContentGapsSection({ admin = false }: ContentGapsSectionProps) {
  ...
  return (
    <section className="rounded-2xl border border-dashed border-cx-forest-dark/20 bg-cx-forest-dark/[0.02]">
      ...
      >
        <div>
          <p className="text-sm font-semibold text-cx-forest-dark">Content gaps</p>
        ...
        <span className="text-xs font-medium text-cx-forest-dark/55">{open ? "Hide" : "Show"}</span>
      </button>
      {open && (
            ...
            return (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  {admin && !isComingSoon && (
                    <p className="mt-0.5 truncate text-xs text-cx-forest-dark/50">{file.url}</p>
                  )}
                  {isSyllabus && (
                ...
                {isSyllabus ? (
                  <a
                    ...
                    className="inline-flex shrink-0 items-center gap-1 text-xs font-medium text-cx-forest-dark underline-offset-2 hover:underline"
                  >
                    Open doc
                    <ExternalLink className="h-3 w-3" aria-hidden />
                  </a>
                ) : (
                  <span className="shrink-0 text-xs text-cx-forest-dark/50">Coming soon</span>
                )}
              </li>
            );
          })}
