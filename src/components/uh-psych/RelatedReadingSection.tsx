  ...
  return (
    <section className="rounded-2xl border border-cx-forest-dark/15 bg-white/80 p-5">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <h2 className="text-base font-semibold text-cx-forest-dark">Related reading</h2>
        <Link
          href="/app/education"
        ...
        {docs.map((doc) => (
          <li key={doc.id}>
            <a
              ...
              className="flex items-start gap-2 py-2.5 text-sm transition hover:text-cx-forest-dark"
            >
              <FileText className="mt-0.5 h-4 w-4 shrink-0 text-cx-forest-dark/45" aria-hidden />
              <span>
                <span className="font-medium text-cx-forest-dark">{doc.title}</span>
                {doc.subcategory && (
                  <span className="mt-0.5 block text-xs text-cx-forest-dark/55">{doc.subcategory}</span>
                )}
              </span>
            </a>
          </li>
