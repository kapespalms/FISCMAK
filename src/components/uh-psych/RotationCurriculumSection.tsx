        ...
        {pgy_tracks.map((track) => (
          <section key={track.pgy_level} className="rounded-xl border border-cx-forest-dark/10 bg-cx-forest-dark/[0.02] p-4">
            <h3 className="text-sm font-semibold text-cx-text">
              {track.pgy_level}
              {track.portal_track ? (
                <span className="ml-2 font-normal text-cx-text/60">({track.portal_track})</span>
              ) : null}
            </h3>
            {track.role && (
              <p className="mt-1 text-xs text-cx-text/65">{track.role}</p>
            )}

            <div className="mt-3 overflow-x-auto">
              <table className="min-w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-cx-forest-dark/10 text-cx-text/55">
                ...
                <tbody>
                  {Object.entries(track.milestone_targets).map(([code, targets]) => (
                    <tr key={code} className="border-b border-cx-forest-dark/5 align-top">
                      <td className="py-2 pr-4 font-medium text-cx-text/80">
                        {MILESTONE_LABELS[code] ?? code}
                      </td>
                      <td className="py-2 text-cx-text/75">
            ...
            {track.other_competency_objectives?.length ? (
              <div className="mt-3">
                ...
                <ul className="mt-1.5 list-disc space-y-1 pl-5 text-xs leading-relaxed text-cx-text/75">
