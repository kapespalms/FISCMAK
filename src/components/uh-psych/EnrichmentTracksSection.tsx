  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold text-cx-text">Program paths</h2>
      ...
      <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {UH_PSYCH_ENRICHMENT_TRACKS.map((track) => (
          ...
          >
            <p className="text-sm font-semibold text-cx-text">{track.title}</p>
            <p className="mt-1.5 text-sm leading-relaxed text-cx-text/75">
              {track.description}
            </p>
            <p className="mt-2 text-xs text-cx-text/55">{track.eligibility}</p>
          </li>
        ))}
      </ul>
    </section>
