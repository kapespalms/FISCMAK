...
export function SemiAnnualReviewSection() {
  return (
    <section className="rounded-2xl border border-cx-forest-dark/15 bg-white/90 p-5">
      ...
      <h2 className="mt-1 text-lg font-semibold text-cx-forest-dark">
        Meeting with {SEMI_ANNUAL_MEETING.hosts}
      </h2>
      <p className="mt-2 max-w-2xl text-sm text-cx-forest-dark/75">{SEMI_ANNUAL_MEETING.cadence}</p>
      ...
      <ul className="mt-4 space-y-2">
        {SEMI_ANNUAL_REVIEW_ITEMS.map((item) => (
          <li key={item.id}>
            ...
            >
              <Circle
                className="mt-0.5 h-4 w-4 shrink-0 text-cx-forest-dark/35 group-hover:text-cx-forest-dark/55"
                aria-hidden
              />
              <div>
                <p className="text-sm font-medium text-cx-forest-dark">{item.label}</p>
                <p className="text-xs text-cx-forest-dark/65">{item.detail}</p>
              </div>
            </Link>
          </li>
        ))}
