    ...
    <header
      className={
        overlay
          ...
          : "sticky top-0 z-50 border-b border-cx-forest-dark/10 bg-[#FCFBF7]/90 backdrop-blur-xl"
      }
    >
      <nav
        ...
        <div className="marketing-glass pointer-events-auto flex items-center gap-1 rounded-full px-2 py-1.5">
          {navigationItems.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className="font-futura-medium rounded-full px-4 py-2 text-sm text-cx-text/70 transition hover:bg-cx-forest-dark/5 hover:text-marketing-accent"
            >
              {item.label}
            </Link>
          ))}
      ...
      <div className="relative mx-auto flex max-w-6xl items-center justify-between px-6 py-5 md:px-10">
        ...
        <div className="relative z-10 ml-auto flex shrink-0 items-center gap-3">
          <Link
            href="/login"
            className="font-futura-bold cx-btn hidden border border-cx-forest-dark/20 bg-white/60 px-5 py-2.5 text-sm text-cx-text backdrop-blur-sm transition hover:border-marketing-accent/60 hover:bg-white/80 sm:inline-block"
          >
            Sign In
          </Link>
          <Link
            href="/app/onboarding"
            className="font-futura-bold cx-btn bg-marketing-accent px-5 py-2.5 text-sm text-white shadow-[0_4px_16px_rgba(172,134,54,0.3)] transition hover:bg-fis-gold/90 hover:shadow-none"
          >
            Start Building
          </Link>
        </div>
