  ...
  return (
    <section aria-label="How FISCMAK works" className="relative px-6 py-16 md:px-10 md:py-20">
      <div className="relative mx-auto max-w-6xl">
        <h1 className="font-futura-bold text-3xl text-cx-text md:text-4xl lg:text-5xl">
        ...
        <MarketingGlassPanel accent className="relative mt-10 overflow-hidden p-8 md:p-12">
          <div
            className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_0%_50%,rgba(172,134,54,0.05),transparent_60%)]"
            aria-hidden
          />
          ...
          <ol className="relative space-y-0">
            {FLOW_STEPS.map((step, index) => (
              <li
                key={step.n}
                className="fiscmak-flow-detail group grid grid-cols-1 gap-4 border-b border-cx-forest-dark/[0.08] py-8 last:border-b-0 md:grid-cols-[5rem_1fr] md:gap-8 md:py-10"
              >
                <div className="flex items-start md:justify-center">
                  <span className="font-futura-bold text-2xl text-marketing-accent/70 transition-colors duration-300 group-hover:text-marketing-accent md:text-3xl">
                    {step.n}
                  </span>
                </div>
                <div>
                  <h2 className="font-futura-bold text-xl text-cx-text md:text-2xl">{step.title}</h2>
                  <p className="font-futura-medium mt-3 max-w-2xl text-sm leading-relaxed text-cx-text/60 md:text-base">
                    {step.body}
                  </p>
                </div>
                {index < FLOW_STEPS.length - 1 ? (
                  <div
                    className="col-span-full hidden h-px bg-gradient-to-r from-marketing-accent/20 via-cx-forest-dark/5 to-transparent md:block"
                    aria-hidden
                  />
                ) : null}
              </li>
        ...
        <MarketingGlassPanel className="mt-8 p-6 md:p-8">
          <p className="font-futura-medium text-sm leading-relaxed text-cx-text/65 md:text-base">
            Residency and GME programs use FISCMAK for cohort dashboards, evaluation import, and
            review prep —{" "}
            <Link href="/institutions" className="text-marketing-accent transition hover:text-fis-gold/80">
              see Institutions
            </Link>
            .
          </p>
