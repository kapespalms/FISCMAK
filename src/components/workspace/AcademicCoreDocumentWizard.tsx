...
export function AcademicCoreDocumentWizard({
  ...
  if (loading || !data || !sectionMeta) {
    return <p className="text-sm text-cx-forest-dark/70">Loading {docDef.label}…</p>;
  }
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4 lg:flex-row">
      <aside className="w-full shrink-0 space-y-3 lg:w-72">
        <div className="rounded-xl border border-cx-forest-dark/15 bg-cx-forest-dark/[0.03] p-3">
          <p className="text-xs font-semibold uppercase text-cx-forest-dark/70">{docDef.label}</p>
          <p className="mt-1 text-xs text-cx-forest-dark/60">{docDef.description}</p>
          <button
            type="button"
            onClick={() => setFormatOpen((o) => !o)}
            className="mt-2 text-left text-xs font-medium text-cx-forest-dark/70 hover:text-cx-forest-dark"
          >
            {formatOpen ? "Hide" : "Show"} formatting notes
          </button>
          {formatOpen && (
            <ul className="mt-1 list-disc space-y-0.5 pl-4 text-xs text-cx-forest-dark/60">
