...
function LevelBadge({ level }: { level: number | null }) {
  if (level == null) {
    return <span className="text-xs text-cx-forest-dark/35">—</span>;
  }
  return (
...
export function MilestoneLatticeRollup() {
  ...
  if (loading) {
    return <p className="text-sm text-cx-forest-dark/50">Loading CCC prep…</p>;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        {!hasRatings && (
          <p className="mt-3 rounded-lg border border-cx-forest-dark/10 bg-cx-forest-dark/[0.03] px-4 py-2.5 text-sm text-cx-forest-dark/70">
        ...
        <div className="mt-3 divide-y divide-cx-forest-dark/8 overflow-hidden rounded-xl border border-cx-forest-dark/10 bg-white/90">
          {data.skills.map((skill) => {
            ...
            return (
              ...
              >
                {/* Skill name */}
                <span className="w-52 shrink-0 text-sm text-cx-forest-dark">
                  {skill.skill_name}
                </span>
                ...
                {skill.total_count > 0 && (
                  <div className="flex flex-1 items-center gap-2">
                    ...
                    <span className="w-14 shrink-0 text-right text-[10px] text-cx-forest-dark/45">
        ...
        <p className="mt-2 text-xs text-cx-forest-dark/50">
