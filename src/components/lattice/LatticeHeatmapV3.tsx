...
export function LatticeHeatmapV3({ prefetchedData, onCellClick }: Props) {
  ...
  if (loading) {
    return <p className="text-sm text-cx-text/50">Loading lattice…</p>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs tracking-wide text-[#9A968C]">
      ...
      <div className="overflow-x-auto">
        <div
          ...
          style={{ gridTemplateColumns: `90px repeat(${NUM_DOMAINS}, minmax(40px, 1fr))` }}
        >
          ...
          {DOMAIN_SHORT.map((domain) => (
            <div
              key={domain}
              className="px-1 py-2 text-center text-[8px] font-medium leading-tight tracking-wider text-[#9A968C]"
            >
              {domain}
            </div>
          ))}
          ...
          {Array.from({ length: NUM_SKILLS }, (_, si) => (
            <Fragment key={si}>
              <div className="flex items-center pr-3 text-right text-[9px] font-medium tracking-wide text-[#9A968C]">
                {SKILL_SHORT[si]}
              </div>
              {Array.from({ length: NUM_DOMAINS }, (_, di) => {
                ...
                // Green glow + gold ring so the reserved hero genuinely pops as precious.
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "relative h-10 w-full rounded-md border text-[9px] font-semibold transition-all hover:ring-1 hover:ring-[#9A968C]/40",
                      dn === 0 && "border-[#ECE8DF] bg-[#F4EFE6]/40 text-transparent",
                      selected?.skill_index === si && selected?.domain_index === di && "ring-2 ring-[#AC8636]",
                      isOI && "ring-2 ring-[#AC8636] ring-offset-1",
                    )}
                    style={
                      dn > 0
                        ? {
                            ...
                            ...fteBorder,
                            ...(isOI
                              ? { boxShadow: "0 0 0 3px #AC8636, 0 0 10px 4px rgba(60,138,96,0.22)" }
                              : {}),
                          }
                        : fteBorder
                    }
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center gap-4 text-xs text-[#9A968C]">
      ...
      {selected && (
        <div className="rounded-xl border border-[#ECE8DF] bg-[#F4EFE6]/60 px-4 py-3">
