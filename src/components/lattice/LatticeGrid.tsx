  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto">
        ...
        >
          ...
          {DOMAINS.map((domain) => (
            <div
              key={domain}
              className="px-1 py-2 text-center text-[10px] font-semibold leading-tight text-cx-text/60"
            >
              {domain.split("/")[0]}
            </div>
          ))}
          {SKILLS.map((skill, di) => (
            <Fragment key={skill}>
              <div className="flex items-center pr-2 text-right text-[10px] font-medium text-cx-text/60">
                {skill.split(" ")[0]}
              </div>
              {DOMAINS.map((_, ti) => {
      ...
      {selected && selected.activityCount > 0 && (
        <Card>
          <h3 className="text-lg font-semibold text-cx-text">
          ...
          <p className="mt-4 text-sm text-cx-text/80">
