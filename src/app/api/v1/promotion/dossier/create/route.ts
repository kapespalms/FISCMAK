...
export async function POST(request: Request) {
  ...
  const trackId = normalizePromotionTrack(target_track);
  ...
  const row = {
    ...
    target_track: dossierLabelForTrack(trackId),
  ...
  return jsonOk({ dossier_id: dossierId, track_id: trackId, initial_assessment: row.domain_scores }, 201);
}
