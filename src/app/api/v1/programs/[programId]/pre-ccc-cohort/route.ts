...
/** Advisor cohort pre-CCC summaries — alias of pre-ccc/batch with canonical route name. */
export async function GET(
  ...
  try {
    ...
    const summaries = await Promise.all(
      traineeIds.map((traineeUserId) =>
        buildTraineePreCccSummary({
          programId,
          traineeUserId,
    ...
    const message = err instanceof Error ? err.message : "Could not build cohort pre-CCC.";
