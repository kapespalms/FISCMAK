...
export default function StudioPage() {
  ...
  async function downloadDocx() {
    ...
    downloadBlob(blob, `${selected}_${new Date().toISOString().slice(0, 10)}.docx`);
  ...
  async function downloadPdf() {
    ...
    downloadBlob(blob, `${selected}_${new Date().toISOString().slice(0, 10)}.pdf`);
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4">
        ...
        <Card className="flex min-h-0 flex-1 flex-col overflow-hidden p-0">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-fiscmak-border px-4 py-2 text-sm text-fiscmak-muted">
            <span className={overLimit ? "text-fiscmak-amber font-medium" : ""}>
